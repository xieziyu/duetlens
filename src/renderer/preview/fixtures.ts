...
const FINDINGS: Finding[] = [
  ...
  mkFinding({
    ...
    round: 2,
    lastSeenRound: 2,
    // 对抗档自检判它不成立:严重度与去留一个都没动,只多出一枚徽标与一段判据
    verdict: 'refuted',
    verdictNote: 'src/worker.rs:142 的 Drop impl 已经 join 了线程句柄,回收路径是有的。',
    verdictTurn: 'selfcheck',
    verdictRound: 2,
    originTurn: 'scan',
  }),
  mkFinding({
  ...
  'd-f1': [
    ...
    },
    {
      ...
      text: [
        ...
        '它保证单变量原子性,开销接近裸加。要点:',
        '',
        '1. `fetch_add` 本身不可被打断,**不会丢更新**',
        '2. Relaxed 不约束跨变量的可见顺序',
        '3. 只读近似值时这层约束用不上',
        '',
        '```rust',
        ...
        '### 最小止血方案',
        '',
        '- 把 `done` 换成 `AtomicUsize`',
        '- 读侧照旧,`load(Relaxed)` 即可',
      ].join('\n'),
      createdAt: now + 2000,
    },
  ],
