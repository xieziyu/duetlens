...
- **分支是下拉,不是卡片列表**:候选(普通分支 / 虚拟分支)收进一个下拉选择器 —— 平铺成卡片时没人看得出「要点选」,而且没有默认值时底部 CTA 一直是灰的,像是流程卡住了。**进面板即有默认选中**:普通 git 取 `HEAD` 所在分支(不在候选里则第一条),虚拟分支取第一条;换仓库 / 改 base / 切模式后按同规则重挑。收起后由一行摘要补回被收进去的信息(`branch ← base` · 领先数 · 最新 commit · 时间),展开带筛选框以应付分支多的仓库。候选为空时下拉禁用并就地说明原因。
...
- 画面语汇取自应用图标:**镜片在一叠 diff 代码行上来回扫**,镜片下的行才「看清」(亮色副本按镜片圆裁切);已完成的阶段数 = 已点亮的行数。这幅动画抽成共享组件(`components/LensScanArt`),凡是「agent 正在通读改动」的等待画面都用它 —— 另一处是 review 右栏扫描期的零 finding 空态,见下节。
...
- 扫描结束自动切回 Discussion / Findings tab;review 状态随之落定 —— github-pr 转「审核中」(还剩提交 PR 这一步),本地分支 / vbranch 直接转「已完成」(见 [ui-states](ui-states.md#review-生命周期scan--reviewing--completed))。
- **扫描期零 finding 用镜片扫描动画占位**(与启动浮层同一份 `LensScanArt`,点亮行数 = 已完成阶段数):这一屏空得最久,一行短提示既撑不住等待,也容易被读成「干净通过」的结论 —— 后者是扫描**结束**后零 finding 才给的正向空态,两者必须长得不一样。
...
- **状态栏 status 胶囊**(底部状态栏最左):基线是 review 状态(扫描中天蓝 / 审核中·已完成·已提交绿 / 失败红,见 `screens/review/StatusBar.tsx`),运行时态在其上覆盖文案与配色 —— 运行中(天蓝 pulse)/ 待审批(琥珀)/ turn 失败 · agent 已断开 · 回传通道故障(红)/ 离线(琥珀)/ 压缩中(天蓝转圈)。
...
- **右栏底部运行区**(随态切内容):
  ...
  - **5 turn 失败**:错误卡(简述 + 可展开 stream error 详情)+ **重试这一轮 / 编辑后重发**;保留追问与既有对话。**这一支只管追问失败** —— 整轮机审失败没有可挂靠的对话线程,归机审进度条,见下。
...
### 一轮机审失败:归进度条,不归状态栏(2026-07-25)

失败是**轮次**的属性,不是一枚全局状态字。落点因此定在横跨三栏的机审进度条 `.scanbar`(`screens/review/ScanProgressBar.tsx`),而不是底部状态栏:

- **进度条失败后不卸载**。原实现 `status !== 'scanning'` 即整条摘掉,把唯一能表达"断在哪一步"的组件连同断点一起抹了 —— 剩下的只有状态栏一枚红字「失败」,没有原因、没有出口。现在失败时整条转红留在原位,stepper 停在断点(实心红 ✕,与 done 的实心蓝勾成对)。
- **展开区换内容**:跑动时是"扫描期能做什么"的提示,失败时换成失败卡 —— 结论 → 处置建议 → 「本轮没有产出,此前各轮的结论与你的处置都还在」→ `<details>` 里的 agent 原文(等宽、可选中复制)→ 「↻ 重试第 N 轮」。翻进失败态强制展开:原因藏在收起的横条里等于没写。
- **文案按归因分档**(`screens/review/round-error.ts`):用量耗尽 / 上下文超限 / 上游过载 / 连接中断 / 登录失效 / 请求被拒。可重试的那几档「重试」是主按钮,其余是次按钮 —— 明知重试无用还把它做成主按钮,是在骗用户按。原文一律另行原样呈现,不改写、不吞证据。
- **状态栏胶囊降为指路牌**:失败时可点、带一句结论 + 「查看原因」,点了让失败卡展开并闪一下。它不承载原因,只负责在你视线在底部时把你带回去。
- **agent 内部重试要可见**:codex 自己会退避重试若干次,这几十秒原本是纯黑盒。进度条上给一枚琥珀「连接中断,agent 重试中 · 第 n 次」(次数由事件数出,codex 不上报序号)。
- **「开跑前就失败」是另一类**:source 拉不到 / gh 掉线 / 仓库被移走时,agent 根本没被叫起来,没有归因可落库,只有底层命令原文。这一类走 `components` 级的 `LaunchError`(`screens/review/LaunchError.tsx`),**重跑面板与失败卡共用同一份** —— 两处各写各的必然漂移。按可辨认的特征串给人话结论(最常见的是 vbranch 被合掉后 `but diff` 报 `No ID found for entity`),认不出就只给通用结论;原文一律收进 `<details>` 原样呈现,不靠猜替代证据,也不再把一整行 `Error invoking remote method …` 糊进段落。
...
### diff 上批注:提问与记 finding 合成一张卡(2026-07-26 重设计)

"在 diff 上说话"的核心入口。**只有一枚动作、一张卡** —— 行内悬停 ✎ 或框选后点「✎ 段落批注」,都在锚点下方就地插入同一张 composer。

- **finding 是提问的超集,不是并列的另一件事**:卡默认只有选区引用块 + 正文 + `发送给 agent`;footer 右端的 `⚑ 记为 finding` 开关一开,就地长出 severity / category / 标题 / suggestion,正文原地变成「说明」,主 CTA 换成 `新增 finding`。升格是**加法不是切换**,视线不跳,且与后端 `promoteDiscussion`(discussion → finding,锚点沿用、历史保留)是同一条语义。
- **升格时正文首行提为标题,降回时并回首行**:两向对称、一个字都不丢,免得用户为了 `title` 必填而把同一句话再打一遍。首行超长(>120 字)则整段留在说明里,标题交给用户。
- **曾按"来路决定产物"分叉,已废**:行内 `＋` 只能发起 discussion、要记 finding 必须先框选 —— 同一个锚点因为来路不同能做的事不同,而单行记 finding 恰恰是常用场景。弹层里预先分叉也等于逼用户在打字前就下结论(那时往往还没想清是疑问还是问题)。
- **popover 只剩一枚动作**,显示选区 `file:行范围` + `✎ 段落批注`,与行内 ✎ 同一套 human 琥珀配色。浮层本身保留:框选常常只是想复制代码,不该直接弹卡。定位在选区首行上方,贴顶则翻转到选区下方;点击别处 / 滚动即消失。
- **横向恒定贴代码区左缘**(箭头随之固定),不跟选区中点走:那样位置会随所选代码的长度忽左忽右,长行更会把动作条甩到视野之外。
- **曾有第三个动作 `◆ 追问 codex`(切右栏 + 把选区作为引用 chip 附到 composer),已删**:它与就地提问产出完全一致(同锚点、同一条新 user discussion、同一次首问),差别只是在哪里打字;就地那条离代码更近。**新讨论因此只有中栏一条发起路径**,右栏 composer 退化为纯追问活跃线程。
...
- **铅笔画成 SVG,不用 `✎`(U+270E)**:那枚字形的墨迹只占 em 盒一小撮,落进行号格会缩成一个点,字号再大也救不回来。
...
- **diff 内容检索(`⌘F`)**:中栏右上浮一枚检索条(`.dfind`),编辑器式:全部命中着色 + 当前项换色 + `↵`/`⇧↵` 前后跳。与文件过滤(`⌘⇧F`,只匹配路径)分工明确 —— **⌘F 找内容,⌘⇧F 找文件**。
  - **范围 = 全部文件的 diff 行 + 已展开的 diff 外上下文**,跨文件而非只当前文件;不含文件路径、finding 卡、Discussion、右栏。展开上下文会让命中总数增长,故**当前项按身份(文件+行+列)钉住而非按下标**,展开一段上下文不会把你甩到别处去。
  - **索引建在行模型上,不扫 DOM**:折叠 / 已看的文件不在 DOM 里,扫 DOM 会让它们的命中凭空消失,而「跨全部改动找一个符号」正是折叠之后最需要的。跳到折叠文件里的命中时先自动展开(走只展不收的 `expandFile`)。
  - **着色走 CSS Custom Highlight API**,不包 `<mark>`:diff 代码是 highlight.js 产出的 HTML,包标签意味着每次按键重渲整棵 diff。Highlight 只画字、不改 DOM,故 `::highlight()` 里只有 `background` / `color` 一类属性有效,圆角边框无效。命中上限 5000,超出计数标 `5000+`。
  - **跳转要同时滚两个方向**:长代码行的命中常落在中栏右侧之外,只滚纵向的话行是对的、词却看不见,读起来就像「报了命中却找不到」。横向滚的是该行所在的 `.code-scroll`(各表 scrollLeft 本就同步),判定用命中区间自己的 rect —— 整格的 rect 是长行全宽,判不出词在不在视野里;左边界还要让开 unified 下钉住的行号 + gutter,滚到它们底下等于没滚。
  - **检索条不能放进滚动容器**:放进去它会随内容滚开,点上一处 / 下一处时搜索条自己先跑掉。故中栏外面套一层 `.diff-col` 作定位基准,检索条 `position:absolute` 挂在上面。代价是会盖住最上方 file-header 右侧一小块(sticky 的那枚)——这是浮层形态的既定取舍。
  - **两枚开关**:`Aa` 区分大小写(关时为 smart-case:查询含大写才区分)、`ab|` 全词匹配;不做正则。不敏感匹配走**长度守恒**的小写折叠,不能直接 `toLowerCase()` 整行 —— Unicode 折叠不保证长度守恒(`İ` 折成 `i̇`,一个码点变两个),而命中偏移随后要当成原文本的 UTF-16 下标去建 Range,长度一变轻则高亮画到别的字符上、重则越界(命中计数在,却既画不出也跳不过去)。代价是 `İ` 不与 `i̇` 互相匹配。中栏被两侧栏挤到 440px 以下时两枚整组收起(容器查询,按中栏实宽而非视口),保住输入框与前后跳。
...
- **统一快捷键体系 + 帮助层**:散落在各交互里的键位(编辑 `e`、保存 / 发送 `⌘↵`、取消 `Esc`)收敛为一套,并有一个随时可唤起的 cheatsheet 浮层。底部状态栏右端放一个 `⌘ 快捷键` 触发按钮,快捷键 `?` 唤起 / 关闭;`Esc` 关闭。浮层双列分组(通用 / Diff 视图 / Finding / Discussion),键位用 `<kbd>` 呈现,底部注明「⌘ 在 Windows / Linux 为 Ctrl · 带 ⌘ 的导航键在输入框内同样生效」。浮层跟随两轴配色(tokens 驱动),明暗自洽;组件与键位表只有一份,设置屏直接复用(见下「设置 / 偏好」)。
- **键位约定**:导航键一律带 `⌘` 修饰 —— `⌘1`/`⌘2`/`⌘3` 切右栏 Discussion/Findings/Summary、`⌘U` 切 Unified/Split、`⌘F` 查 diff 内容、`⌘⇧F` 聚焦文件过滤框;裸键只留 `?`(帮助)与 `Esc`(关闭弹层 / 取消编辑)。检索条内 —— `↵`/`⇧↵` 前后跳(焦点在别处时用 `⌘G`/`⌘⇧G`)、`Esc` 关闭。文件过滤框内 —— `↵` 跳首个筛选结果、`↑`/`↓` 在筛选结果中移动、`Esc` 先清空再退焦。编辑 / Discussion —— `⌘↵` 保存 / 发送、`Esc` 取消、`↵` 换行(多行追问常见,不让裸 `↵` 误发)。拖选批注、悬停行 ✎ 批注是鼠标手势,列在浮层作参照。
- **让位原则**:带 `⌘` 的导航键在输入框内照常生效(修饰键就是为此),只有裸键 `?` 在焦点落在 `INPUT/TEXTAREA/SELECT` 或 contenteditable 时不拦截;**任何模态**打开时都不抢导航键 —— 判据必须覆盖全部模态(帮助层 + 重跑面板),只认帮助层时在重跑说明框里按 `⌘F` 会把焦点抢到对话框背后的检索条。**这条对组件自带的全局键位同样成立**:检索条的 `⌘G` / `⌘⇧G` 挂在自己的 window 监听上,不接屏侧那份 handler,故要显式接收模态状态并让位,否则弹层开着按 `⌘G` 会在 aria-modal 背后换命中并滚动 diff,关掉才发现视口已经不是原来那处。(实现项:键位表按用户可配置。)
...
- **底部状态栏**(28px,仅 review 屏,参照 IDE / Claude Desktop):左起 **状态胶囊**(扫描中/审核中/已完成/已提交/失败,运行时带 pulse)· `codex · 模型 · effort`(模型名是 agent 侧实际生效的那个:用户没指定时由起会话的应答回填落库)· **ctx 环 + token 用量** · 最近工具调用(最弱、限宽、窄窗口先隐);右端 `⌘ 快捷键`。
...
- **左右栏可拖动调宽**:两条分隔线拖拽,**双击回默认宽度**(拖歪了没有别的路回来);上界收在「两侧同时拉满、中栏仍排得下 diff」,下界收在「侧栏还认得出自己是什么」——具体数值以 `screens/ReviewScreen.tsx` 的 `LEFT_/RIGHT_MIN|MAX` 为准,不在这儿抄一份。由 CSS var `--left-w` / `--right-w` 驱动;栏宽按用户持久化(全局偏好而非 per-review,分层见 [frontend-components](frontend-components.md#ui-状态持久化))。
...
- **左栏分组导航 + 右栏分节表单**,滚动定位联动高亮;每个设置一行(标题 + 说明 + 控件)。落地本地即时保存,底部 `恢复默认设置` 一键复位全部 per-user 偏好(**含栏宽**,单栏复位另有分隔线双击,见「布局与栏宽」);**明暗与配色两轴不在其中** —— 那是用户当下正看着的东西,别被一键掀掉。
- **偏好(per-user)**对齐 `ui_settings`(见 [frontend-components](frontend-components.md#ui-状态持久化)):
  - **外观**:明暗模式(跟随系统 / 浅 / 深)× 配色主题(Duetlens / GitHub)两正交轴,控件**实时驱动 `data-mode`/`data-theme`**。
  ...
  - **快捷键**:键位只读,不再另抄一份摘录 —— 一个按钮复用审核屏 `?` 的同一个帮助浮层组件。
- **环境配置**:
...
- 点卡进入对应 review 屏:状态只反映机审与提交进度,已完成 / 已提交的照样能继续追问、重跑复审。
