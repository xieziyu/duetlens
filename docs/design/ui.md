...
- **键位约定**:导航键一律带 `⌘` 修饰 —— `⌘1`/`⌘2`/`⌘3` 切右栏 Discussion/Findings/Summary、`⌘U` 切 Unified/Split、`⌘F` 查 diff 内容、`⌘⇧F` 聚焦文件过滤框;裸键只留 `?`(帮助)与 `Esc`(关闭弹层 / 取消编辑)。检索条内 —— `↵`/`⇧↵` 前后跳(焦点在别处时用 `⌘G`/`⌘⇧G`)、`Esc` 关闭。文件过滤框内 —— `↵` 跳首个筛选结果、`↑`/`↓` 在筛选结果中移动、`Esc` 先清空再退焦。编辑 / Discussion —— `⌘↵` 保存 / 发送、`Esc` 取消、`↵` 换行(多行追问常见,不让裸 `↵` 误发)。拖选发起 discussion、悬停行 ＋ 追加讨论是鼠标手势,列在浮层作参照。
