...
**一轮自检大致让整次审核的 token 翻倍**,不是"边角开销"。四次实测里自检轮相对扫描轮的输出在 35%–121% 之间(有一次自检比扫描还贵),累计 token 增幅 +71%~+143%。所以定长两轮不是因为便宜,恰恰是因为不便宜:桌面交互工具要的是**可预测**的开销,loop-until-dry 那种收敛式循环把一个已经翻倍的量变成开口的,换来的覆盖率不值这份不确定。

**别拿缓存命中率当省钱的证据。** 自检轮的输入 92%~97% 命中前缀缓存,那说的是延迟与"若缓存计价打折"下的账单;而 `total_token_usage` 的口径是含每轮重发的 cached input,配额侧是否打折是上游黑箱。按能观测到的口径写用户文案,不要按对我们有利的那个假设写 —— `INTENSITY_HINTS` 曾据单次实测写成「多三到四成」,那是把 n=1 里最便宜的一次当成了常态。

样本仍小(4 次自检轮、两个仓库),这里给的是量级不是常数。
...
- **拒收文案要能自救**。agent 手上还有 shell,完全可能 `cat` 过这个文件再来裁决 —— 那时一句「还没取证」在它看来是工具坏了,它会重试同一次调用或干脆放弃。文案要点破口径(别的方式读过不算)并把能解锁的那次调用原样写出来。同理,记账与查账的**路径写法必须归一**:记的是 agent 传来的 `path`,查的是 finding 落库的 `file`,`./a` 与 `a` 不归一就会错杀一次真取了证的裁决,而模型只看到「还没取证」,并不知道差在写法上。
- **取证读的是 diff 的新侧那棵树,不是工作区**。这在 GitButler 下尤其要紧:workspace 的工作区是**所有 applied lane 合并后的样子**,审 A 时 B 的改动就摊在那儿(实测 `cat b.txt` 拿到另一条 lane 的内容,`git show A:b.txt` 则正确地报 not in 'A'),而 diff 只含 A 自己的提交 —— 读工作区等于让 agent 拿着一份混了别人改动、且比被审 diff 更新的代码去核实,行号还可能对不上。顺带一个安全面:树读**不跟随符号链接**,被审仓库里提交的 `leak -> ~/.ssh/id_rsa` 读回的是那行链接目标字符串而非私钥,越界外泄不再靠一道 realpath 判断兜着。被审的那棵树在 `prepare` 时**钉成 sha**,不按分支名逐次重解析 —— GitButler 的日常就是 amend / absorb / squash,按名字重解析会让后半程读到的树与开头那份 diff 不是同一棵。**那份 diff 同样只取一次**,并且必须与钉住的 head 同源:一次审核活得比一次 amend 久(MCP 的 `get_diff` 与 `get_file` 会在会话中途各问各的),而缺省档只有 `but diff <branch>` 给得出「这条分支自己的提交」、它认的又是名字 —— 每次现拉就会回到新树,与还钉在旧树上的 `get_file` 分家。取快照的时机分三种:取过了就不再动;还没有任何取证读过树时分支前进,重钉即可(等价于晚一点 `prepare`);取证已经读过再前进,只能报错 —— 那时改口径会让已经发给 agent 的行号失真。`prepare` 自己**只钉树、不拉 diff**:同一条路还服务「会话已释放时展开 diff 上下文」,那边是一个文件建一次 source,顺手拉整份 diff 会把按需读退化成每读一个文件全量 diff 一次。名字这层还有一处塌陷:短名的消歧顺序把 `refs/tags` 排在 `refs/heads` 之前,仓库里存着同名 tag 时 git 只给一句 ambiguous 警告、`but diff` 更是**静默回一份空 changes**。故解析一律写全 `refs/heads/<name>` 并剥到 `^{commit}`,缺省档遇上同名 tag 宁可挡住,也不让整轮审核在「零改动」上跑完。
...
`search_code` 只在 source 拿得到完整代码树、且能与 `get_file` **同口径**时才声明:local-branch 与 gitbutler 都搜**被审那一侧的 commit 树**,**github-pr 不实现**(走 `gh api` 逐文件取内容,没有可搜的树)。口径不一致比没有搜索更坏 —— agent 会拿着对不上的两段代码推出一个像模像样的结论。不实现时工具**根本不声明**,而不是声明了再报错:工具不存在,agent 就不会调用,也就不会把「搜不了」误读成「代码里没有」。
