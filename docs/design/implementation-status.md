...
>
> **剩余 backlog(前端)**:核心项已全部收口(见文末「后续可选项」为非核心的 3 项 nice-to-have)。已落地:第三层打磨(**a11y 焦点管理** + **窄窗口退化**)、审核规则三层编辑器(`PromptRulesScreen` + `prompt:get/save`)、提交屏 per-finding 422 定位 + fix-action(本地锚点预判 + `review:set-finding-anchor`)、第三层空态与 diff 边界打磨(Findings 干净空态 + diff 文件状态 pill)、**模型选择 + reasoning effort**(EntryScreen 选择器 + 落库 + 注入 codex thread/start)、**长任务完成通知**(扫描完成/追问回复:失焦弹原生系统通知·点击聚焦打开 review,聚焦弹应用内 toast + 偏好开关)。
...
前端核心 backlog 已收口;以下 3 项为非核心 nice-to-have,均因「更重 / 与既有决策冲突 / 设计未定」暂缓,做前先确认:

1. **模型下拉动态拉取 codex `models`**:EntryScreen 现为手填模型名(留空=账号默认)。改为下拉需 codex 登录 + 新 RPC(`models` 列举)+ 真烧 token,非纯前端切片。
2. **per-review `last_active_tab` 持久化**:与「tab 改为全局 `ui_settings` 偏好」的既有决策冲突,`review_ui_state.last_active_tab` 列现有意留空;若做等于回退该决策,需先确认是否要 per-review 记忆 tab。
3. **prompt 节内追加(非整节替换)**:当前分层为「每节整节替换」winner-takes-all;改为节内可追加涉及规则合并语义取舍,design 层「待议」。
