...
>
> **剩余 backlog(前端)**:审核规则三层编辑器(`mockup/prompt-rules.html` + 读走 `review-prompt.ts` / 写落 `.duetlens/review.md`)· 提交屏 per-finding 422 定位 + fix-action · 第三层打磨(空态/窄窗口/模型选择/a11y/通知)。
...
7. 前端:diff-review 三栏真实屏(进行中,已合入 main = 3b2f704)
   ...
   - ⏳ 提交屏 per-finding 422 定位 + fix-action(改锚点 / 降级为摘要 / 剔除):GitHub 422 不告知是哪条锚点失效,现只在 banner 泛提示;后续可结合最新 diff 的新侧行号本地预判哪条 finding 锚点已失效并高亮
