...
7. 前端:diff-review 三栏真实屏(进行中,已合入 main = 3b2f704)
   ...
   - ✅ 提交屏 · 导出 Markdown(非 GitHub source 终点,feat/dev):`SubmitExportScreen` 按 `review.source` 分派——`github-pr` 仍占位(提交屏后续),`local-branch`/`gitbutler-vbranch` → `ExportMarkdownScreen`(左预览 hero + 右配置)。纯生成器 `src/shared/export-markdown.ts`(`buildReviewMarkdown`/`exportFileName`/`isKept`,triage!=dismiss 即保留、按严重度/文件分组、summary/suggestion/dismissed 开关),轻量 md 渲染 `screens/export/markdown.ts`(渲染/源码切),右栏保留 checklist 走同一 triage 管线(`setTriage`→事件回推→预览实时刷新)。保存经新 IPC `dialog:save-text-file`(main `showSaveDialog`+`writeFile`);复制走 `navigator.clipboard`。`npm run spike:export` 确定性 PASS;preview 双主题实测:渲染/源码/分组/开关/checklist 剔除回环/已剔除段全通。**GitHub 提交屏(gh 原子 review + 422/failed/增量态)留后续切片。**
   - ⏳ gutter 锚点圆点(有讨论的行)· 扫描 timeline · 顶栏合并 · 手动新增 finding · GitHub 提交屏
