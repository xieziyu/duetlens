...
>
> **剩余 backlog(前端)**:第三层打磨(空态/窄窗口/模型选择/a11y/通知)。审核规则三层编辑器(`PromptRulesScreen` + `prompt:get/save`)、提交屏 per-finding 422 定位 + fix-action(本地锚点预判 + `review:set-finding-anchor`)均已落地。
