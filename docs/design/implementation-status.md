...
> 入口发起(本地分支 / GitHub PR / GitButler 三 source)→ **真实 codex 首轮扫描** → findings 经 MCP 流入 → 三栏 diff review(unified/split · 语法高亮 · viewed/折叠 · 拖栏宽 · 文件检索过滤 · ⌘F diff 内容检索)→ triage(保留/剔除+可选理由/就地编辑)→ 框选/行内批注(一张卡:提问 ⇄ 升格为 manual finding)+ **追问 codex**(多轮 · 重启后 `thread/resume` 续接)→ **↻ 重跑复审**(每轮新 thread · 重拉 diff · agent 对旧 findings 表态 · 剔除项抑制 · 同步 PR 评论)→ Summary(结论/统计/可编辑总结)→ 终点:**GitHub PR review 原子提交**(真实 `gh api`,进屏与 422 后均**现拉最新 diff**定位失效锚点,支持逐条/成批修锚)或 **导出 Markdown**。
...
- **导航快捷键一律带 `⌘`,帮助层只有一份**:`1/2/3`、`u`、`/` 这类裸键在一屏全是输入框的 review 上要么误触、要么打字时被迫失效,改为 `⌘1/⌘2/⌘3`、`⌘U`、`⌘F`、`⌘⇧F`(裸键只留 `?` 与 `Esc`),修饰键在输入框内照常生效。设置屏原先手抄一份「常用摘录」,已漂移出 `e` 编辑、`⌘.` 中断、`⌘⇧↵` 提交三条从未实现的键位 —— 现在改为一个按钮复用 `components/KbdHelp` 同一浮层(键位只读,无第二份表可抄错)。
- **⌘F diff 内容检索:索引建在行模型、着色走 Highlight API、检索条在滚动容器之外**。三条各有一个非做不可的理由:(1) 扫 DOM 建索引会让**折叠 / 已看文件的命中凭空消失**,而跨全部改动找一个符号正是折叠之后最需要的,故索引取自 `files[].hunks[].lines[]` 再叠上已展开的 diff 外上下文 —— 后者要求把上下文展开状态从 `DiffFileView` 提到 `DiffPane`,子组件里的局部 state 索引取不到;(2) diff 代码是 highlight.js 产出的 HTML,包 `<mark>` 意味着每次按键重渲整棵 diff,改用 CSS Custom Highlight API 后零 DOM 改写、零重渲,代价是 `::highlight()` 只吃 `background` / `color` 一类属性;(3) 检索条放进 `.diff` 会随内容滚开 —— 实测哪怕用零高度 sticky,它也会在头 30px 滚动里先滑一段才钉住,点上一处 / 下一处时搜索条自己先跑偏,故外套一层 `.diff-col` 作定位基准。另有一个自踩的坑:「跳到当前项」的 effect 挂在 `collapsed` 上(展开后才滚得到),若展开走 `onToggleCollapsed` 这种 toggle,用户手动折叠恰好含当前命中的文件时会被本 effect 立刻掰回去 —— 必须走只展不收的 `expandFile`,并用 `pendingReveal` 把「真的请求了跳转」与「collapsed 恰好变了」区分开。命中总数随上下文展开而增长,故当前项按身份(文件 + 行 + 列)钉住而非按下标;反过来,**钉不住就必须跟着滚过去** —— `Aa` / 全词把当前项筛掉时只换下标不换视口,计数显示的是新的一处、屏幕停在旧位置。上下文缓存提到整栏后活得比一条 review 长(切 review 只换 prop、组件不卸载),故按 `files` 引用当版本号在 render 期派生作废,并把版本随 `fetchFileContent` 请求一起带走,免得上一条 review 迟到的全文写进新一版。另外 Highlight 存的是 Range,任何重建 `.src` 单元格的状态(换视图、展开文件、findings 到达、开关行内 composer)都必须触发重画,否则计数还在而高亮凭空消失。
