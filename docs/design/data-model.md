...
## PR 里的提交范围 = 子 review 行

一次 PR review 可以在「整个 PR」与「PR 里的某一个提交」之间切换(见 [ui](ui.md#pr-内的审核范围顶栏-chip-下拉))。每个范围落成**一条子 review 行**:`parent_review_id` 指向 PR 那条,`head_ref` 是被钉住的 sha。

**为什么是子行,而不是一条 review 挂多份 diff。** 一个范围拥有的东西恰好是一条 review 的全部:自己的 diff 快照、轮次履历、findings、codex 会话、已看进度、提交记录。挂在同一行下面就要给轮次、findings、`review_ui_state` 全部再加一列范围键,而它们的**每一处**读写都得记得带上它 —— 漏一处就是把另一个范围的 finding 算进本范围的待提交集,而那批是会真的发到 PR 上的。

**`current_round = 0` 表示「只拉了 diff,还没机审」。** 不为它另加一枚 `status`:状态字说的是「agent 那边现在怎么样」,而这里要说的是「还没开始过」——混进同一枚枚举,每个读 status 的地方都得再分一次「是没跑还是跑完了」。轮次号本就从 1 起,0 是它天然的空位。判据收在 `domain.isUnscanned`,状态栏 / CTA / 空态三处一律问它。

**两个范围的 findings 互不合并,各自提交。** 行号基准不同(一个相对 PR base,一个相对该提交的父提交),合起来的列表里同一个 `file:line` 指向两个东西;而 GitHub 一份 review 只认一个 `commit_id`,跨范围提交在协议层就不成立。提交屏因此只提一句「其它范围还有 N 条待提交」,不做跨范围提交。

**保留期只判顶层行,子行随父级联删。** 一个提交范围的存活由它所属的 PR 决定 —— 逐行判的话,几周前看过、之后没再动的那个范围会先于 PR 消失,而 PR 还在列表里:点进去范围少了几个,界面上没有任何地方解释得了。为了让这条成立,**子行的每一次 touch 都要把父行的 `updated_at` 一并推上去**(与子表冒泡同一条约定,见下节),否则父行的时间会早于子行,整个 PR 连同还在被追问的那个范围一起过期。

**列表一行一个顶层 review。** 子行不单独出现在入口 / 历史里 —— 它们是同一个 PR 的几个视角,各占一行会让「最近审过什么」被同一个 PR 的七八条刷屏。计数(findings / 讨论 / 已提交)因此按容器 + 全部子行合计:不合计的话,一条 PR 上明明有 findings、行上却写着 0(它们都记在子行名下)。
