...
Review
  ...
  └─ rounds[]  (review_rounds 表)
        ...
        errorMessage / errorKind   ← 失败留证:agent 原文 + 中立归因
        changedFiles / codeChanged ← 本轮变更基线;重试同一轮时沿用
        startedAt / endedAt
...
**关键约定**:`resolution` 只在 `lastSeenRound === Review.currentRound` 时代表**本轮**结论。第 2 轮判定 `still_present` 的条目,到第 3 轮若 agent 没再表态,就该回到"未表态",而不是一直挂着旧结论。唯一的例外是 `isAutoClosedFixed` 的自动结案:它按约定后续轮次不再被要求表态,故跨轮有效(见下文 UI 一节)。派生逻辑收在 `renderer/screens/review/rounds.ts`,不散落在各组件里。

`dismissReason` 是**事后可选补充**,不是剔除时的必填门槛 —— 一键剔除的速度不能被输入框拖慢,但填了理由下一轮的同类抑制会准得多。

schema 见 `src/backend/db/schema.ts` 的 V6(轮次表)与 V9(失败留证 + 变更基线);存量数据一律视作第 1 轮(列默认值),无需回填 —— 但 V9 之前的失败轮次原因已不可追溯。
...
失败原因**必须落库**。它在 agent 侧只存在一瞬(`turn/completed` 的 `turn.error`),编排层不接住就永久丢失 —— 用户只剩状态栏一句「失败」,重启后连这句都没有出处。故 `settleRound` 在失败分支把 `AgentTurnError` 的原文与归因一并写进轮次行。

归因是 **provider 中立**的(`AGENT_ERROR_KINDS`,见 `shared/agent-events.ts`),不直接暴露 codex 的错误码 —— UI 的处置建议按「用户能做什么」分档(等额度 / 换模型 / 查网络 / 重新登录 / 稍后重试),而不是按某家 agent 的错误分类分档。映射见 `codexErrorKind`,未知一律归 `other`,不按 message 猜。

`retryRound(reviewId)` 重试**同一轮号**,不新开一轮:失败那次没有任何产出,再给它一个轮号只会让「第 N 轮」退化成重试计数。落地靠 `startRound` 的 upsert —— 同轮号重开即覆盖该行、清零统计与失败痕迹。代价是失败那次的记录被覆盖,这是有意的取舍。

变更文件基线因此要单独存:失败那一轮已经把最新 diff 覆盖进快照,重试时再比一次只会得出"无改动",复审 prompt 就会睁眼说瞎话。故 `changedFiles` / `codeChanged` 落在轮次行上,重试取并集。

**不做自己的自动重试** —— codex 内部已有退避重试(默认 5 次);再兜一层只会让失败更慢、更难解释。可重试的错误由用户点「重试本轮」。但 agent 重试期间要**可见**:`error` 通知里 `willRetry=true` 的那些映射成 `turn-retrying` 事件外发,否则这几十秒是纯黑盒。
...
- **本轮失败**:机审进度条(`.scanbar`)**不卸载**,转红留在原位 —— 它是唯一横跨三栏、能同时承载"断在哪一步 / 为什么断 / 怎么重试"的位置;状态栏只放得下一枚状态字,把原因塞那儿等于没有原因。stepper 停在断点(✕),展开区从"扫描期能做什么"换成失败卡:一句话结论 → 处置建议 → 「本轮没有产出,此前各轮的结论仍在」→ 可展开的 agent 原文(可选中复制)→ 「↻ 重试第 N 轮」。翻进失败态时强制展开。
- **状态栏那枚「失败」胶囊**变可点,带一句结论 + 「查看原因」——它只是指路牌,点了让上面的失败卡展开并闪一下。
- **开跑前失败**(source 拉不到 / gh 掉线 / 仓库被移走)不进轮次记录 —— 那一轮压根没开始,`startRound` 还没跑到。它就地反馈给发起处:重跑面板或失败卡里的重试按钮,两处共用 `LaunchError`。最常见的一档是 **vbranch 被合入目标分支后 `but diff <branch>` 报 `No ID found for entity`**:此时这条 review 只能只读查看与导出,要再跑一轮需先把对应分支 apply 回 workspace。
...
- **复核说明会被提交出去**:判定 `still_present` 的条目提交到 GitHub / 导出报告时,以本轮复核说明作正文主体,首轮正文降为背景;上一轮已提交的还会就同一处**追发一条复核评论**(每轮至多一条)—— 见 [findings-submit.md](findings-submit.md)。
- **两个折叠区**:已有结论的条目移出主列表 —— 留在原位只会淹没真正待处理的意见。`✓ 复核判定已修复` 默认收起;`◇ 作者已回应,未改动` **默认展开**,因为它还等着 reviewer 决定接不接受作者的说法。
- **已修复跨轮有效**:其余表态只在「表态轮次 === 当前轮次」时算数,`fixed` 自动结案是例外 —— 结案的条目按约定后续轮次不再被要求表态,若也按本轮口径算,它下一轮就会掉回主列表、顶着删除线冒充待处理项。折叠区里的历轮条目标 `✓ 第 N 轮已修复 · 自动剔除`。reviewer 手动剔除后 agent 才判 `fixed` 的那种没有 `autoClosed`,仍按本轮口径处理:那条剔除是他的判断。
...
`npm run spike:rerun`(确定性、不烧 token):轮次落库与级联删除、失败留证(原因可重新读出)与同轮重开(重试不新增轮次、沿用说明与变更基线、清掉上次失败痕迹)、`changedFilesBetween`、复审 prompt 的六类内容与外部数据围栏顺序、表态词汇含 `wont_fix` 且判定顺序把作者回应排在代码之前、thread↔finding 匹配**不依赖数组顺序**、去重的命中/不误吞/跨文件、三态回写(`fixed` 自动结案且不覆盖 reviewer 已填的剔除理由、`wont_fix` 不自动剔除)与一键采纳、结案节与剔除节在 prompt 里分开且留有回归出口、抑制计数、`lastSeenRound` 不回退。
