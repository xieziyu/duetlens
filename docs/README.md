...
codex app-server 把「一次常驻会话」本身称作 `thread`。为避免冲突,Duetlens 把自己「锚定代码的讨论线程」统一叫 **discussion**;`thread` / `conversation` 只指 agent 侧的会话实体(codex 的 thread、pi 的 session)。
...
| [pi-integration.md](design/pi-integration.md) | pi 接入:子进程 RPC 与 extension 桥的取舍、只读保证换了什么性质、阶段判据 | 碰 pi 集成 / 加第二个 agent 前 |
...
2. **Electron + Node/TS 主进程后端 + codex app-server / pi rpc 子进程 + 外部 `gh` CLI**,先做 macOS。选 Electron 是为渲染与 Chrome 一致。→ [architecture](design/architecture.md)
...
4. **一轮机审 = 一个 agent 会话**(轮内全局视野),**跨轮换新会话**,上一轮结论靠结构化 prompt 注入。→ [rerun](design/rerun.md)
...
6. **只 review,不改代码**:codex 锁 read-only sandbox,pi 只给 Duetlens 自己的只读工具;没有「让 agent 给修法」;`suggestion` 只作提给 author 的 GitHub suggestion 块。→ [findings-submit](design/findings-submit.md)
7. **每次 review 选一个 agent(codex / pi),建行即定死**;两条链路各自直连 `ConversationalAgent`,不共享中间件;同一次 review 内多 agent 协同不做。→ [architecture](design/architecture.md) · [pi-integration](design/pi-integration.md)
