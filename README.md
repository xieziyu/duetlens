...
Duetlens 是一个 macOS 桌面应用:把一次 code review 变成你和 agent(codex 或 pi)的**双人对话**,而不是读一份机审报告。
...
**只审,不改。** codex 跑在 read-only sandbox 里,pi 手上只有 Duetlens 给的只读工具,没有"让它顺手修一下"这个选项。`suggestion` 只作为提给作者的 GitHub suggestion 块。
...
- macOS,Apple Silicon
- 审核 agent 二选一,装好一个即可,每次审核在入口选用哪个:
  - [codex CLI](https://github.com/openai/codex) 并已 `codex login` —— 经 `codex app-server` 驱动(在 0.144.x / 0.145 实测)
  - [pi](https://github.com/earendil-works/pi/tree/main/packages/coding-agent) 并已配好至少一个 provider(订阅登录或 API key) —— 经 `pi --mode rpc` 驱动
...
2. 开始审核:Duetlens 拉 diff、起一个常驻 agent 会话跑首轮机审。findings 经 Duetlens 提供的工具回传,边扫边落到 diff 上,不用等扫完。
3. 追问、triage、自己补 finding。想换个强度或换个模型再看一遍,就重跑一轮。
4. GitHub PR 提交 review;本地分支导出 Markdown。

每一轮机审都在花你 codex 账号或 pi provider 的额度(API key 按量计费),底部状态栏常驻显示模型、effort 和上下文用量。

## 设计与文档

设计目标、已拍板的决策及其理由在 [docs/README.md](docs/README.md);工程约定在 [CLAUDE.md](CLAUDE.md)。

技术栈:Electron + React + TypeScript,后端写在 main 进程;审核 agent 是 codex app-server 的常驻 JSON-RPC 会话或 pi 的 rpc 子进程;findings 走进程内自托管的 HTTP MCP server 回传(pi 经一个薄 extension 桥过来);本地存储 better-sqlite3。
